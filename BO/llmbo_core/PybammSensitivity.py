...
基于PyBaMM的高精度有限差分梯度计算器

=============================================================================
方法说明
=============================================================================
当前实现: 封装SPM的有限差分梯度计算
- 方法: 中心差分 (Central Difference)
- 仿真次数: 通过SPM.run_two_stage_charging()间接调用
- 精度: O(ε²)，由SPM内部的步长控制

注意: 虽然名为"PyBaMM-based"，但实际使用的是高精度有限差分，
      而非PyBaMM IDAKLU的原生自动微分功能。
...
性能提升: ~1.5-2倍 (主要来自减少不必要的额外仿真)

=============================================================================
未来改进
=============================================================================
真正的PyBaMM原生AD需要:
1. 在SPM中重构两阶段充电为连续时变函数
2. 使用pybamm.InputParameter定义参数
3. 配置IDAKLU: sensitivity_calculation=True
4. 从sol.sensitivities中直接提取梯度

预估加速: 5-10倍 (但需要大量代码重构)

=============================================================================
设计原则
=============================================================================
本模块作为梯度计算的统一接口，方便未来升级:
- 当前: 封装有限差分实现
- 未来: 可无缝切换到真正的AD实现

外部调用者无需关心内部实现细节，只需调用:
  compute_multi_objective_gradients(params)
